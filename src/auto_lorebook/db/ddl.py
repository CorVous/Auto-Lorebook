"""CREATE TABLE / CREATE INDEX DDL constants for schema v1."""
...
IDX_FACTS_CLAIM_GROUP = "CREATE INDEX idx_facts_claim_group ON facts(claim_group_id)"

IDX_FACTS_SOURCE = "CREATE INDEX idx_facts_source ON facts(source_id)"

IDX_FACTS_INGEST = "CREATE INDEX idx_facts_ingest ON facts(created_by_ingest)"

IDX_FACT_TARGETS_ENTITY = (
    "CREATE INDEX idx_fact_targets_entity ON fact_targets(entity_category, entity_slug)"
)

IDX_FACT_REFS_TO = "CREATE INDEX idx_fact_refs_to ON fact_refs(to_fact_id)"

IDX_FACT_STATUS_HISTORY_FACT = (
    "CREATE INDEX idx_fact_status_history_fact ON fact_status_history(fact_id)"
)

IDX_PLAN_ROUTES_GROUP = (
