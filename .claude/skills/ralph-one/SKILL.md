---
name: ralph-one
description: Drive a single user-supplied issue end-to-end through a plan → implement → review → smoke → PR loop. Plans once with Opus, then iterates implement/review with Sonnet subagents on the issue's branch in the main checkout until approved, runs an integration smoke, opens a PR, and closes the issue on merge. Use when user invokes /ralph-one or asks to autonomously work a single issue from triage-ready to merged.
---
